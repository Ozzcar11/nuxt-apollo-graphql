...
local: ## Подготовка проекта для локальной разработки
	...
	./docker/scripts/docker-mirrors.sh
	$(DOCKER_EXEC) build
...
install: ## Установка node_modules
	$(DOCKER_EXEC) run --rm $(CONTAINER_NAME) $(PACKAGE_MANAGER) install
...
rm: ## Останавливает текущий контейнер и удаляет .env и docker compose файлы
	...
	rm -rf src/.pnpm-store

test:
	docker-compose exec app $(PACKAGE_MANAGER) run test ${file}

test-watch:
	docker-compose exec app $(PACKAGE_MANAGER) run test:watch

test-coverage:
	docker-compose exec app $(PACKAGE_MANAGER) run test:coverage
