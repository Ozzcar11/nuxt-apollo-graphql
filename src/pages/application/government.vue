...
import { applicationBusiness, applicationPersonal } from "@/router/application";

import { type Area } from "@/models/application";

const router = useRouter();
const stepperItems = Object.values(APPLICATION_STEPS);
const stepperModel = ref<number>(2);
...
onResult(() => {
  ...
  applicationDataParent.ref.value = cloneDeep(applicationTree.value?.find(item => item.name === APPLICATION_STEPS.GOVERMENT));
  const applicationDataValue = applicationDataParent.ref.value?.children ?? [];

  for (const item of applicationDataValue) {
    for (const question of item.application_area_questions) {
      if (!question.user_application_questionnaires.length && question.system_name !== "social_investments") {
...
async function syncQuestions(toNextPage: boolean = false, questionEdit: boolean = false) {
  ...
  if (toNextPage) {
    void router.push(applicationBusiness);
  }
}
